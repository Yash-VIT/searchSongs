...
$form.submit(function (evt, theForm) {
    ...
    fetch(url)
        ...
        .then(function (songData) {
            $('#results ul')
                .html(buildList(songData.results));
            $('#results ul')
                .find('li.song').click(function (evt) {
                    ...
                    $('#favourites ul').html(buildList(favSongs));
                })
        })
        .catch(function (err) {
            alert('ERROR!');
...
:
      ...
      >
        ...
        <a href="${item.trackViewUrl}" target="_blank" ><img src="${item.artworkUrl100 || item['artworkUrl-100']}" width=30" height="30" /></a>
...
function setupFavoriteList() {
    ...
    $('#favorites ul')
        .html(buildList(JSON.parse(favSongs)))
}
